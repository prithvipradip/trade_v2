...
OUT = ROOT / "reports" / "shadow_tournament_uncorr"
OUT.mkdir(parents=True, exist_ok=True)
SYMBOLS = ["XLE", "GLD", "TLT"]
ARMS = ['iron_condor', 'iron_butterfly', 'put_credit_spread', 'call_credit_spread', 'jade_lizard', 'short_strangle']
...
async def run_k(strat, bc, data):
    ...
    try:
        ...
        pass
    print(f"[shadow2] {strat}: {m}", flush=True)
    return m
...
def load_data():
    ...
    for sym in SYMBOLS:
        ...
        if df is not None and len(df):
            ...
            data[sym] = df
            print(f"[shadow2] data {sym}: {len(df)} rows", flush=True)
    return data
...
async def main():
    ...
    for strat in ARMS:
        try:
            ...
            print(f"[shadow2] {strat} FAILED: {e}", flush=True)
    ...
    print("[shadow2] PLAN rule: promote only if PF > 1.2 AND max_dd_pct <= IC arm, n >= 30; strangle = benchmark only")
