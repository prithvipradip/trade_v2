...
OUT = ROOT / "reports" / "range_floor_study"
OUT.mkdir(parents=True, exist_ok=True)
SYMBOLS = ["SPY", "QQQ", "IWM"]
# R16 RANGE-FLOOR STUDY: gates ON for every arm; sweep the floor that
# is now the binding live constraint (settings.ml.range_min_confidence).
# NOTE the parity gap this study also closes: WalkForwardConfig defaults
# to 0.55 while LIVE runs 0.65, so the 08-08 confirm run measured a
# LOOSER gate than production actually applies.
ARMS = [('floor_0.50', 0.50), ('floor_0.55', 0.55), ('floor_0.60', 0.60),
        ('floor_0.65', 0.65), ('floor_0.70', 0.70)]
