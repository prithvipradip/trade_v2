...
Writes reports/shadow_round3_rerun/<arm>.json + table."""
import asyncio
import json
import re
import sys
...
OUT = ROOT / "reports" / "shadow_round3_rerun"
...
async def run_k(strat, bc, data):
    cfg = WalkForwardConfig(
        wing_k=0.8,
        train_window_models=False,  # R16 RERUN: no window models at all —
        # matches LIVE (entry_gates_enabled=False) and makes the
        # look-ahead leak structurally impossible. The original run
        # scored OOS windows with the live future-trained ensemble.
