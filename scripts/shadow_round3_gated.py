...
Writes reports/shadow_round3_gated/<arm>.json + table."""
import asyncio
import json
import re
import sys
...
OUT = ROOT / "reports" / "shadow_round3_gated"
OUT.mkdir(parents=True, exist_ok=True)
SYMBOLS = ["SPY", "QQQ", "IWM"]
ARMS = ['iron_condor', 'wide_wing_condor', 'broken_wing_condor']
...
async def run_k(strat, bc, data):
    cfg = WalkForwardConfig(
        wing_k=0.8,
        train_window_models=True,  # R16 GATES-ON RERUN: live now runs
        # entry_gates_enabled=True (rule B1), so the wing arms must be
        # compared under the SAME regime. Fence still on (no live
        # artifact fallback); train_days lengthened so models train.
    ...
    try:
        ...
        pass
    print(f"[shadow] {strat}: {m}", flush=True)
    return m
...
def load_data():
    ...
    data = {}
    for sym in SYMBOLS:
        df = load_daily_ohlcv(sym, days=4000)
        if df is not None and len(df):
            if getattr(df.index, "tz", None) is not None:
                df = df.copy(); df.index = df.index.tz_localize(None)
            data[sym] = df
            print(f"[shadow] data {sym}: {len(df)} rows", flush=True)
    return data
...
async def main():
    ...
    for strat in ARMS:
        try:
            ...
            print(f"[shadow] {strat} FAILED: {e}", flush=True)
    ...
    print("[shadow] PLAN rule: promote only if PF > 1.2 AND max_dd_pct <= IC arm, n >= 30; strangle = benchmark only")
